...
class Plot:
    ...
    def __init__(self, opts, Video, grid, step):
        ...
        if not (isinstance(self.D_groundTruth, float)):
...
# =============================================================================
#         plt.tight_layout()
#         plt.savefig('/home/florent/tmp/D_vs_DGroundTruth_%s.jpg' % self.itern)
    ...
    def results(self, opts, grid, Results, Kalman, InvStg, d_lims, diff_lims, err_lims, gc_kernel, fft_spectrum,
        ...
        self.D = np.reshape(Kalman.derrt_prev[0, :], (grid.Numrows, grid.Numcols), order='F')
        self.D_diff_stat = np.flipud(self.D) - self.D_groundTruth
        self.d_bias = np.append(self.d_bias, np.nanmean(self.D_diff_stat))
        self.d_rmse = np.append(self.d_rmse, np.sqrt(np.nanmean(self.D_diff_stat ** 2)))
        D_diff_stat2 = self.D_diff_stat - np.nanmean(self.D_diff_stat)
        self.d_mrmse = np.append(self.d_mrmse, np.sqrt(np.nanmean(D_diff_stat2 ** 2)))
        ...
        dKerrors = Kalman.derrt_prev[1, :]
        dKerrors = np.flipud(np.reshape(dKerrors, (grid.Numrows, grid.Numcols), order='F'))

        u = np.flipud(np.reshape(Kalman.uerrt_prev[0, :], (grid.Numrows, grid.Numcols), order="F"))
        v = np.flipud(np.reshape(Kalman.verrt_prev[0, :], (grid.Numrows, grid.Numcols), order="F"))
        ...
        #         if self.iter_now % 2 == 1:
        # =============================================================================
        im1 = self.ax1.imshow(np.flipud(self.D), extent=[grid.X[0, 0], grid.X[0, -1], grid.Y[0, 0], grid.Y[-1, 0]], cmap='jet_r',
                              origin='lower')  # 'terrain_r'
        CS1 = self.ax1.contour(grid.X, grid.Y, self.D_groundTruth, levels=[0.5, 2, 3, 4, 5, 6],
        ...
        im2 = self.ax2.imshow(self.D_diff_stat, extent=[grid.X[0, 0], grid.X[0, -1], grid.Y[0, 0], grid.Y[-1, 0]],
                              cmap='Spectral_r', origin='lower')
        CS2 = self.ax2.contour(grid.X, grid.Y, self.D_diff_stat, levels=[-1.5, -0.5, 0.5, 1.5], colors=('k',),
                               linewidths=(1.5,))
        self.ax2.clabel(CS2, CS2.levels, fontsize=14, fmt='%.1f')  # , inline=True

        im3 = self.ax3.imshow(dKerrors, extent=[grid.X[0, 0], grid.X[0, -1], grid.Y[0, 0], grid.Y[-1, 0]],
                              cmap='Spectral_r', origin='lower')
        # =============================================================================
        #         im4     = self.ax4.imshow(n_used_layers,extent = [grid.X[0,0],grid.X[0,-1],grid.Y[0,0],grid.Y[-1,0]], cmap = 'Spectral_r', origin = 'lower')
        # =============================================================================
        ...
        norm = Normalize(vmin=0, vmax=len(Results.c_omega))
        skip = (
        slice(None, None, round(grid.Numrows / 30 * density)), slice(None, None, round(grid.Numcols / 30 * density)))
        for ii in range(len(Results.c_omega)):
            cc = cm.jet(norm(ii), bytes=True)
            im4 = self.ax4.quiver(grid.X[skip], grid.Y[skip],
                                  np.flipud(np.reshape(Kalman.cxerrt_prev[0, :, ii], (grid.Numrows, grid.Numcols), order='F')[
                                      skip]),
                                  np.flipud(np.reshape(Kalman.cyerrt_prev[0, :, ii], (grid.Numrows, grid.Numcols), order='F')[
                                      skip]),
                                  color=(cc[0] / 255, cc[1] / 255, cc[2] / 255, cc[3] / 255))

        im5 = self.ax5.streamplot(grid.X[0, :], grid.Y[:, 0], u, v, color=np.sqrt(u ** 2 + v ** 2), linewidth=1,
        ...
        if np.all(np.isnan(self.D_groundTruth)):
            ...
            im7 = self.ax7.scatter(self.D_groundTruth[~np.isnan(self.D_groundTruth)],
                                   np.flipud(self.D)[~np.isnan(self.D_groundTruth)], 1.5, color=self.color2)
        self.ax7.plot(d_lims, d_lims)
        ...
        fft_freqlims_ID = (fft_spectrum[1] / (2 * np.pi) > opts.freqlims[1]) & (
                fft_spectrum[1] / (2 * np.pi) < opts.freqlims[0])
        ...
        self.ax1.axis('equal');
        self.ax1.axis('tight');
        self.ax1.set_title('d [m]');
        self.ax1.set_ylabel('y [m]');
        self.ax1.set_xlabel('x [m]')
        self.ax2.axis('equal');
        self.ax2.axis('tight');
        self.ax2.set_title('diff. [m]');
        self.ax2.set_ylabel('y [m]');
        self.ax2.set_xlabel('x [m]')
        self.ax3.axis('equal');
        self.ax3.axis('tight');
        self.ax3.set_title('Kalman d-error [m]');
        self.ax3.set_ylabel('y [m]');
        self.ax3.set_xlabel('x [m]')
        self.ax4.axis('equal');
        self.ax4.axis('tight');
        self.ax4.set_title('c(T) [m/s]');
        self.ax4.set_ylabel('y [m]');
        self.ax4.set_xlabel('x [m]')
        self.ax5.axis('equal');
        self.ax5.axis('tight');
        self.ax5.set_title('U [m/s]');
        self.ax5.set_ylabel('y [m]');
        self.ax5.set_xlabel('x [m]')
        self.ax6.set_title('med.bias(red) and IQR(blue)');
        self.ax6.set_ylabel('IQR [m]');
        self.ax6b.set_ylabel('med.bias [m]');
        self.ax6.set_xlabel('update [#]')
        self.ax7.set_title('Direct comp.');
        self.ax7.set_ylabel('d_inv [m]');
        self.ax7.set_xlabel('d_meas [m]')
        self.ax8.axis('equal');
        self.ax8.axis('tight');
        self.ax8.set_title('SSPC sample locs.');
        self.ax8.set_ylabel('gc_y [-]');
        self.ax8.set_xlabel('gc_x [-]')
        self.ax9.axis('equal');
        self.ax9.axis('tight');
        self.ax9.set_title('FFT(green) vs. DMD(orange) spectra');
        self.ax9.set_ylabel('A [norm. intensity]');
        self.ax9.set_xlabel('omega [rad/s]')
        self.ax10.axis('equal');
        self.ax10.axis('tight');
        self.ax10.set_title('CPU time per update');
        self.ax10.set_ylabel('t-CPU [s]');
        self.ax10.set_xlabel('update [#]')
        ...
        plt.pause(0.05)
        plt.tight_layout()
        self.fig.savefig('/home/florent/tmp/results_COCOS_palavas_cristal_1_cpu_speed_%s_%02d.jpg' %(opts.CPU_speed,
                                                                                                   int(self.itern[-1])))
        plt.pause(0.05)

    def results_only_bathy(self, grid, Kalman, t_shift):
        self.iter_now = self.iter_now + 1
        self.itern = np.append(self.itern, self.iter_now)
        self.t_iter = np.append(self.t_iter, t_shift)
        self.D = np.reshape(Kalman.derrt_prev[0, :], (grid.Numrows, grid.Numcols), order='F')
        ...
        im = ax.pcolor(grid.X, grid.Y, np.flipud(self.D), cmap='jet_r', vmin=0, vmax=8)
        f.colorbar(im, ax=ax)
        plt.show()
