...
# =============================================================================
        ...
        # im1_bis = ax.imshow(self.D, extent = [grid.X[0,0],grid.X[0,-1],grid.Y[0,0],grid.Y[-1,0]],cmap = 'jet_r', origin = 'lower',
        #                     vmin=0, vmax=10)
        ax.imshow(self.D, extent=[grid.X[0, 0], grid.X[0, -1], grid.Y[0, 0], grid.Y[-1, 0]], cmap='jet_r',
                            origin='lower', vmin=0, vmax=8)
        # ~ from mpl_toolkits.axes_grid1 import make_axes_locatable
        # ~ divider = make_axes_locatable(ax)
        # ~ cax = divider.append_axes("right", size="5%", pad=0.05)
        # ~ f.colorbar(cax=cax,label=('Depth (m)'))
        # ~ ax.colorbar(im1_bis, ax=ax)
        f.savefig('/home/florent/dev/COCOS/results/bathy/palavas_cristal_1/bathy_iter_%s.jpg' %self.iter_now)
        f.close()
