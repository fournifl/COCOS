...
class Inversion():
    ...
    def get_storage(grid):
        op_print('initialize spectral storage...')
        class SSPC_storage():
    ...
    def get_gridKernel(self, grid, rad = 100):
        ...
        op_print('initialize sampling kernel...')
...
# =============================================================================
    ...
    def get_maps(self, Video, opts, dmd, grid, gc_walk_unwrap_self, storage, t):
        ...
        if opts.parallel_flag:
            op_print('   invoke parallel computation for inversion...')#backend="loky"#require='sharedmem'#prefer = "threads"
            ...
            for gc in range(Ngc):
                op_print('processing grid cells % {:}'.format(int(gc/Ngc*100)))
        ...
        endi  = time.time()
        op_print('CPU time: {} s'.format(np.round((endi-starti)*100)/100))
        # store results
        op_print('gather results...')
        ...
        op_print('store SSPCs of current sequence...', end =" ")
        starti = time.time()
        ...
        op_print(('remove SSPCs older than stationary time = ',opts.stationary_time,'s from temporary storage...'), end =" ")
        starti = time.time()
        ...
        def gc_dm_K_prior_layer(opts, gc_dm = None, p = None, omega = None, dx = None):#, p = None, fft2_sz = None
            ...
            for iii in range(1):# to be able to break if things go 'bad'
                ...
                if (c_prior == 0) & np.all(np.concatenate(np.array([kx_cr])) == 0) & np.all(np.concatenate(np.array([ky_cr])) == 0):
                    op_print('bad k estimates from PIV and 2D-FFT')
                    bad = True
                    break
                ...
                if c_prior < np.sqrt(9.81*min(opts.dlims)):#< np.sqrt(9.81*opts.dlims):#== 0:
                    op_print('bad k estimates from PIV')
                ...
                # else caclulate k from c for PIV and possibly combine with 2D-FFT
                    ...
                    if np.all(np.concatenate(np.array([kx_cr])) == 0) & np.all(np.concatenate(np.array([ky_cr])) == 0):
                        op_print('bad k estimates from 2D-FFT')
