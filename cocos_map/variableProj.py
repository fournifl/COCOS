...
class Varpro():
    def __init__(self,
        ...
        op_print('      opt DMD with max # iterations = {}'.format(maxiter))
        op_print('      opt DMD with full jacobian = {}'.format(fulljac_flag))
    ...
    def checkinputrange(self,xname,xval,xmin,xmax):
        if xval>xmax:
            op_print('Option {:} with value {:} is greater than {:}, which is not recommended'.format(xname,xval,xmin,xmax))
        if xval<xmin:
            op_print('Option {:} with value {:} is less than {:}, which is not recommended'.format(xname,xval,xmin,xmax))
    ...
    def varpro_LM(self, Xvid_T, t, m = None, n = None, iss = None, ia = None, alpha_init = None):
        ...
        printRuntimes   = False
        sigmaRankReduc  = True
        op_print('      use rank reduction: {}'.format(sigmaRankReduc))
        ...
        for itern in range(self.maxiter):
            ...
            if itern>0:
                ...
                if (err[itern-1]-err[itern] < self.eps_stall*err[itern-1]):
                    niter   = itern
                    imode   = 8
                    op_print('stall detected: residual reduced by less than {:} times residual at previous step. \n      iteration {:} \n      current residual {:}'.format(self.eps_stall, itern, errlast))
                    return b, alpha, niter, err, imode, alphas, err_semi, err_side, res, irank
            phimat      = self.varpro2expfun(alpha,t)
            U,S,Vh      = svd(phimat, full_matrices = False)
            S           = np.diag(S); sd = np.diag(S)
...
# =============================================================================
        ...
        niter   = self.maxiter
        imode   = 1
        op_print('      failed to reach tolerance after maxiter={:} iterations \n current residual {:}'.format(self.maxiter,errlast))
        return b, alpha, niter, err, imode, alphas, err_semi, err_side, res, irank
