...
class DMD():
    ...
    def get_times(self, Video, start, stop, stoptype = 'exclude'):
        op_print('create frame timestamps...')
    ...
    def get_Xvid(self, Video, start, stop, fp):
        ...
        def set_Xvid_nanzeroColumns2nans(self):
            op_print('   convert pixel zeros to NaN...')
        ...
        def detrend_Xvid(self):
            op_print('   detrend frame sequence...')
        ...
        def handle_Xvid_nans(self):
            ...
            if self.opts.excl_nan_flag:
                op_print('   exclude fully NaN pixel timeseries...')
        ...
        def get_fourier(self):
            op_print('   get Fourier spectrum for comparison (not necessary)...', end =" ")
            start   = time.time()
            ...
            if self.opts.coast_detect:
                op_print('   try to detect dry areas...', end =" ")
                thr = 0.3
        ...
        def make_Xvid_analytic(self,npix_max = 25_000):
            if self.opts.analytic_flag:
                op_print('   extend to analytic signal...', end =" ")
                start   = time.time()
        ...
        def standing_wave_detect(self):
            if self.opts.standing_wave_flag:
                op_print('   enable standing wave detection...')
...
# =============================================================================
        ...
        endt    = time.time()
        op_print('CPU time: {} s'.format(np.round((endt-startt)*100)/100))
...
# =============================================================================
        op_print('   get U basis of video matrix...', end =" ")
        start   = time.time()
...
# =============================================================================
    def project_Xvid(self, npix_max = 100_000):
        op_print('   project video matrix on U basis...', end =" ")
        start   = time.time()
        iternum     = int(np.ceil(self.Xvid.shape[0]/npix_max))
        if iternum > 1: #for large number of pixels, convert in badges
            self.Xvid_uproj = 0j
    ...
    def filter_frequencybounds(self):
        ...
        op_print('frequency filter Dynamic Modes...', end =" ")
        start   = time.time()
    ...
    def del_weak_modes(self):
        ...
        if np.any(del_IX):
            op_print('delete {} weak Dynamic Mode(s)...'.format(np.sum(del_IX)))
    ...
    def stack_phi(self):
        op_print('stack Dynamic Mode layers...')
    ...
    def get_b_fourier(self):
        op_print('get Fourier compliant spectral amplitudes...')
...
class OptDMD(DMD):
    ...
    def __init__(self, opts, alpha0 = None):#(self,opts):
        op_print('initialize optimized DMD...')
        self.alpha0 = alpha0
    ...
    def get_alpha0(self,rank):
        ...
        op_print(('   find initial Dynamic Mode frequencies via ', self.opts.iniDMD_exact, 'approach...'), end =" ")
        start   = time.time()
...
# =============================================================================
    def get_dynamic_modes(self):
        def get_beta_tilde_alpha(self):
            op_print('   get scaled Dynamic Modes and corresponding frequencies via variable projection...')
            start   = time.time()
...
# =============================================================================
            ...
            if self.irank < self.opts.r_DMD:
                op_print('   rank reduced to {}'.format(self.irank))
            end     = time.time()
            op_print('   CPU time: {} s'.format(np.round((end-start)*100)/100))

        def beta_tilde2phi_b(self):
            op_print('   split scaled Dynamic Modes into Dynamic Modes with spectral weights...', end =" ")
            start   = time.time()
            # normalize
        ...
        try:
            ...
            if (self.opts.calcdmd == 'robust') and self.alpha0 is not None:
                op_print('   robust DMD calculation...')
                ...
                try:
                    ...
                    op_print('   new mimizer failed --> use current minimizer')
                    self.alpha0 = alpha_prev
                if success1 and success2:
                    if err1 < err2:
                        op_print('   new mimizer (res: {}) worse than current minimizer (res: {}) --> use current minimizer'.format(err2,err1))
                        self.alpha0 = alpha_prev
                        get_beta_tilde_alpha(self)  # get projected weighted dynamic modes (beta_tilde) and corresponding frequencies(alpha, including real part representing increase/decrease)
                        success1 = True
                    else:
                        op_print('   new mimizer (res: {}) better than current minimizer (res: {}) --> use new minimizer'.format(err2,err1))
        except:
            self.alpha0 = None          # if previous mimimum raises an error...
            self.get_alpha0(self.opts.r_DMD)           # ...reinitialize alpha0 via exact-like dmd
            get_beta_tilde_alpha(self)  # get projected weighted dynamic modes (beta_tilde) and corresponding frequencies(alpha, including real part representing increase/decrease)
            ...
            if success1:
                op_print('   DMD successfull on current mimimizer')
            elif success2:
                op_print('   DMD successfull on new mimizer')
            else:
